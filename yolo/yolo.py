...
class YoloLayer(Layer):
    ...
    def call(self, x):
        ...
        loss = tf.Print(loss, [grid_h, tf.reduce_sum(input_tensor=loss_xy),
...
def _conv_block(inp, convs, do_skip=True, finetune = 0):
    x = inp
    count = 0
    ...
    for conv in convs:
        if count == (len(convs) - 2) and do_skip:
            skip_connection = x
        count += 1
        #Aqui tenemos en cuenta el finetune para la parte de "Darknet"
        if finetune > 7 and finetune < 82:
            trainable = conv['layer_idx'] > 82 - finetune  # Solo se entrena si estamos en una capa superior a la 78 - el finetune
        else:
            trainable = True
...
def create_yolov3_model(
    ...
    x = _conv_block(x, [{'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 75},
                        ...
                        {'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 79}], do_skip=False, finetune=finetune)
    ...
    # Layer 83 => 86
    x = _conv_block(x, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 84}], do_skip=False, finetune=finetune)
    x = UpSampling2D(2)(x)
    ...
    x = _conv_block(x, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 87},
                        ...
                        {'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 91}], do_skip=False, finetune=finetune)

    # Layer 92 => 94
    pred_yolo_2 = _conv_block(x, [{'filter': 512, 'kernel': 3, 'stride': 1, 'bnorm': True,  'leaky': True,  'layer_idx': 92},
                             {'filter': (3*(5+nb_class)), 'kernel': 1, 'stride': 1, 'bnorm': False, 'leaky': False, 'layer_idx': 93}], do_skip=False, finetune=finetune)
    ...
    # Layer 95 => 98
    x = _conv_block(x, [{'filter': 128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True,   'layer_idx': 96}], do_skip=False, finetune=finetune)
    x = UpSampling2D(2)(x)
